      ...
      S.charts.op = new Chart(document.getElementById('chart-op'), {
        ...
        data: {
          ...
          datasets: [{ data: d.operadoras.map(o => o.count),
            backgroundColor: ['#6B1D3A','#22c55e','#f59e0b','#ef4444','#3b82f6','#8b5cf6','#14b8a6','#f97316'],
  ...
  renderHistChart(hist) {
    ...
    S.histDetailChart = new Chart(cv, {
      ...
      data: {
        ...
        datasets: [
          { label: 'Valor Contratado', data: items.map(h => h.valor_contratado), borderColor: '#6B1D3A', backgroundColor: '#6B1D3A22', fill: true, tension: 0.3, borderWidth: 2 },
...
const Relatorios = {
  ...
  html(comps, ops) {
    ...
    return `
      ...
      <div class="config-card" style="max-width:860px">
        <h3><i class="fas fa-calendar-days" style="color:#6B1D3A"></i> Filtrar Faturas por Data / Número</h3>
  ...
  renderPreview(res) {
    ...
    resultEl.innerHTML = `
      ...
      <div class="fatura-preview-header">
        <h3><i class="fas fa-table" style="color:#6B1D3A"></i> Preview — ${U.esc(res.filename || this.state.filename)}</h3>
